...
@section('title', 'Return Penjualan - Apotekku')
@section('page_title', 'Riwayat Return Penjualan')
...
<style>
    ...
    /* Using Rose/Red for Return Penjualan to distinguish from Return Pembelian while maintaining premium feel */
...
<div class="page-header-banner">
    <div class="header-content">
        ...
        <p>Kelola pengembalian barang dari pelanggan dengan mudah dan transparan.</p>
    </div>
    <div class="page-header-right">
        <a href="{{ route('return-penjualan.create') }}" class="btn-create-p">
...
<div class="table-container">
    <div class="table-responsive">
        <table class="datatable" id="returnTable">
            <thead>
                <tr>
                    <th class="ps-4">No. Return</th>
                    <th>Tanggal</th>
                    <th>Faktur Asal / Pelanggan</th>
                    <th>Petugas</th>
            ...
            <tbody>
                ...
                <tr>
                    <td class="ps-4">
                        <span class="fw-bold text-slate-800 d-block">{{ $ret->no_return_pj }}</span>
                    ...
                    <td>
                        ...
                        <div class="small text-slate-400 font-mono">{{ \Carbon\Carbon::parse($ret->created_at)->format('H:i') }}</div>
                    </td>
                    <td>
                        <a href="{{ route('penjualan.show', $ret->penjualan_id) }}" class="text-link">
                            {{ $ret->penjualan->no_transaksi }}
                        </a>
                        <div class="small text-slate-500 mt-1">
                            <svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="display:inline; margin-right:4px;"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                            {{ $ret->penjualan->pelanggan->nama ?? 'Umum' }}
                        </div>
                    </td>
                    <td>
                    ...
                    <td class="text-end fw-bold text-rose-600">Rp {{ number_format($ret->total_return, 0, ',', '.') }}</td>
                    <td>
                        <div class="action-group">
                            <a href="{{ route('return-penjualan.show', $ret->id) }}" class="btn-action btn-view" title="Lihat Detail">
                ...
                <tr>
                    <td colspan="6" class="text-center py-5">
                        ...
                        <p class="text-slate-500 mb-4">Semua pengembalian barang dari pelanggan akan muncul di sini.</p>
                        <a href="{{ route('return-penjualan.create') }}" class="btn btn-success">Mulai Buat Return</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
...
<script>
    document.addEventListener('input', function(e) {
        if (e.target.id === 'searchInput') {
            ...
            let rows = document.querySelectorAll('#returnTable tbody tr:not(.empty-row)');
            rows.forEach(row => {
